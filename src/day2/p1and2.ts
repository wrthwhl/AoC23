...
const parse = (input: string) => {
  ...
  const games = lines.map(line => {
    ...
    return results.split(";").map(result => {
      const color = result.trim().split(",");
      return color.map(string => {
        const pattern = new RegExp(COLORS.join('|'));
        string = string.trim();
        return ({
          color: string.match(pattern)[0],
          count: parseInt(string.match(/\d*/)[0])
        })
      })
...
const getPossibleGames = (results: ReturnType<typeof parse>) => {
  const possibleGames = [];
  results.forEach((result, i) => {
    // console.log('i:', i)
    const possible = result.every((sample, j) => {
      const possible = conditions.every(condition => {
        const [[testColor, max]] = Object.entries(condition);
        const colorSample = sample.find(({ color }) => color === testColor);
        if (!colorSample) return true;
        // if (colorSample.count > max) console.log("impossible", j, ":", colorSample);
        return colorSample.count <= max;
      })
      return possible;
    })
...
// console.log(getPossibleGames(parse(input)).reduce((sum, num) => sum += num));

const getSmallestPossibleSetOfCubes = (results: ReturnType<typeof parse>) => {
  return results.map(game => {
    ...
    const minSet: Record<typeof COLORS[number], number> = game.reduce((minSet, sample) => {
      sample.map(({ color, count }) => {
        minSet[color] = Math.max(minSet[color], count);
      })
      return minSet
    }, initialSet)
    return minSet;
...
const smallestSets = getSmallestPossibleSetOfCubes(parse(input));
const solution = smallestSets.reduce((sum, game) => sum += Object.values(game).reduce((prod, num) => prod *= num),0);//Object.entries(game).reduce((prod, [_, count]) => prod *= count));

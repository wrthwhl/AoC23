...
const SPELLED_DIGITS: Record<string, number> = {
  one: 1,
  two: 2,
  three: 3,
  four: 4,
...
const parse = (input: string) => {
  ...
  const numbers = lines.map(line => {
    ...
    const pattern = new RegExp(Object.keys(SPELLED_DIGITS).join("|"));

    let match: RegExpExecArray;
    let parsedLine = line;
    while ((match = pattern.exec(parsedLine)) != null) {
      if(match) parsedLine = parsedLine.slice(0,match.index+1) + SPELLED_DIGITS[match[0]].toString() + parsedLine.slice(match.index+1)
    }
    const numbers = parsedLine.match(/\d/g);
    if (!numbers) return null;
    const value = parseInt(numbers[0]) * 10 + parseInt(numbers[numbers.length - 1])
    return value;
  });

  return numbers.filter(Boolean).reduce((sum, number) => sum += number)
}
